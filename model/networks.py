...
from .spectralNormalization import SNConv2d
from .spectralNormalization import SNLinear
import sys
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal',
             gpu_ids=[], n_downsampling=2, padding_type='reflect'):
    netG = None
    ...
    if which_model_netG == 'PATN':
        ...
        netG = PATNetwork(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                           n_blocks=9, gpu_ids=gpu_ids, n_downsampling=n_downsampling, padding_type=padding_type)
    elif which_model_netG == 'Blend':
        from .Generator import BlendNetwork
        netG = BlendNetwork(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    elif which_model_netD == 'Blend':
        netD = BlendDiscriminator(input_nc, ndf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=n_layers_D,
                                   gpu_ids=[], padding_type='reflect', use_sigmoid=use_sigmoid,
                                   n_downsampling=n_downsampling)
    elif which_model_netD == 'res':
        netD = BlendDiscriminator(input_nc, ndf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=n_layers_D,
                                   gpu_ids=[], padding_type='reflect', use_sigmoid=use_sigmoid,
                                   n_downsampling=n_downsampling)
    elif which_model_netD == 'SN':
        netD = NLayerDiscriminatorSN(input_nc, ndf, norm_layer=norm_layer, use_dropout=use_dropout, n_layers=n_layers_D,
                                     gpu_ids=[], padding_type='reflect', use_sigmoid=use_sigmoid,
                                     n_downsampling=n_downsampling)
    elif which_model_netD == 'SN2':
        netD = SNResDiscriminator(input_nc, ndf, norm_layer=norm_layer, use_dropout=use_dropout, n_layers=n_layers_D,
                                     gpu_ids=[], padding_type='reflect', use_sigmoid=use_sigmoid,
                                     n_downsampling=n_downsampling)
    elif which_model_netD == 'SN_NOPATCH':
        netD = SN_NOPATCH_ResDiscriminator(input_nc, ndf, norm_layer=norm_layer, use_dropout=use_dropout, n_layers=n_layers_D,
                                     gpu_ids=[], padding_type='reflect', use_sigmoid=use_sigmoid,
                                     n_downsampling=n_downsampling)
    elif which_model_netD == 'BLENDSN':
        netD = BlendDiscriminatorSN(input_nc, ndf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=n_layers_D,
                                   gpu_ids=[], padding_type='reflect', use_sigmoid=use_sigmoid,
                                   n_downsampling=n_downsampling)
    else:
...
class BlendDiscriminator(nn.Module):
    """Discriminator network with PatchGAN."""

    def __init__(self, input_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[],
                 padding_type='reflect', use_sigmoid=False, n_downsampling=2):
        super(BlendDiscriminator, self).__init__()
        layers = []
        layers.append(nn.Conv2d(input_nc, ngf, kernel_size=4, stride=2, padding=1))
        layers.append(nn.LeakyReLU(0.01))
        self.gpu_ids = gpu_ids
        curr_dim = ngf
        for i in range(1, 5): #原来是5
    ...
    def forward(self, x):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            return nn.parallel.data_parallel(self.main, x, self.gpu_ids)
        else:
            return self.main(x)


class BlendDiscriminatorSN(nn.Module):
    """Discriminator network with PatchGAN."""

    def __init__(self, input_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[],
                 padding_type='reflect', use_sigmoid=False, n_downsampling=2):
        super(BlendDiscriminatorSN, self).__init__()
        layers = []
        layers.append(SpectralNorm(nn.Conv2d(input_nc, ngf, kernel_size=4, stride=2, padding=1)))
        layers.append(nn.LeakyReLU(0.01))
        self.gpu_ids = gpu_ids
        curr_dim = ngf
        for i in range(1, 5): #原来是5
        ...
        layers.append(SpectralNorm(nn.Conv2d(curr_dim, 1, kernel_size=3, stride=1, padding=1, bias=False)))
        if use_sigmoid:
            layers.append(nn.Sigmoid())
        self.main = nn.Sequential(*layers)
...
class NLayerDiscriminatorSN(nn.Module):
    '''
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_layers=6, gpu_ids=[],
                 padding_type='reflect', use_sigmoid=False, n_downsampling=2):
        super(NLayerDiscriminatorSN, self).__init__()
        layers = []
        layers.append(SpectralNorm(nn.Conv2d(input_nc, ndf, kernel_size=4, stride=2, padding=1)))
        layers.append(nn.LeakyReLU(0.01))
        self.gpu_ids = gpu_ids
        curr_dim = ndf
        for i in range(1, 5):
...
class SNResDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_layers=4, gpu_ids=[],
             padding_type='reflect', use_sigmoid=False, n_downsampling=2):
        super(SNResDiscriminator, self).__init__()
        self.res_d = self.make_model(input_nc, ndf, n_layers)
        self.gpu_ids = gpu_ids
    def make_model(self, input_nc, ndf, n_layers):
        model = []
        model += [OptimizedBlock(input_nc, ndf)]
        tndf = ndf
        for i in range(n_layers):
            model += [ResBlock(tndf, tndf*2, downsample=True)]
            tndf *= 2
        model += [nn.ReLU()]
        model += [SNConv2d(tndf, 1, kernel_size=3, padding=1)]
        return nn.Sequential(*model)
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            return nn.parallel.data_parallel(self.res_d, input, self.gpu_ids)
        else:
            return self.res_d(input)

class qlj(nn.Module):
    def __init__(self,ndf):
        super(qlj, self).__init__()
        self.fc = SNLinear(ndf * 16, 1)
    def forward(self, input):
        #print(input.size())
        out = F.avg_pool2d(input, input.size(3), stride=1)
        out = out.view(-1, 1024)
        return self.fc(out)


class SN_NOPATCH_ResDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_layers=4, gpu_ids=[],
             padding_type='reflect', use_sigmoid=False, n_downsampling=2):
        super(SN_NOPATCH_ResDiscriminator, self).__init__()
        self.res_d = self.make_model(input_nc, ndf, n_layers)
        self.gpu_ids = gpu_ids
    def make_model(self, input_nc, ndf, n_layers):
        model = []
