...
def draw_bbox_keypoints(img, bbox, keypoint, must_return=False):
    ...
    if must_return:
        ...
        if flags.count(2) < 0.85*len(flags):
...
def crop_video(anno):
    ...
    os.makedirs("crop_sideface/{}".format(video_name), exist_ok=True)
    os.makedirs("points_sideface/{}".format(video_name), exist_ok=True)
    ...
    for idx, frm in tqdm(enumerate(vid.read())):
        ...
        if points_frm is not None:
            cv2.imwrite("points_sideface/{}/{}.jpg".format(video_name, idx), points_frm)
            cv2.imwrite("crop_sideface/{}/{}.jpg".format(video_name, idx), face)

    for d in os.listdir("points_sideface"):
        full_d = os.path.join("points_sideface", d)
        if len(os.listdir(full_d)) < 8:
            shutil.rmtree(full_d)
    for d in os.listdir("crop_sideface"):
        full_d = os.path.join("crop_sideface", d)
        if len(os.listdir(full_d)) < 8:
            shutil.rmtree(full_d)
