...
class KeyDataset(BaseDataset):
    def initialize(self, opt):
        ...
        self.images = os.listdir('/media/data2/zhangpz/ai_dancing/datasets/blend/')
    ...
    def __getitem__(self, index):
        ...
        to_video_name = input_p_path[16:21]
        to_image_name = input_p_path[21:29]

        input_PB_path = to_video_name + '/poses/' + to_image_name

        BG_index = random.randint(0, 299)
        input_BG_path = to_video_name + '/background/' + '{:04}.jpg'.format(BG_index)

        input_GT_path = to_video_name + '/images/' + to_image_name

        mask_person_path = to_video_name + '/mask/' + to_image_name + '.npy'

        mask_bg_path = to_video_name + '/mask/' + '{:04}.jpg.npy'.format(BG_index)

        input_seg_person_path = to_video_name + '/segmentation/' + to_image_name

        input_P = Image.open(os.path.join('/media/data2/zhangpz/ai_dancing/datasets/blend/', input_p_path)).convert('RGB')
        ...
        return {'P': input_P_t, 'PB': input_PB_t, 'BG': input_BG_t, 'GT': input_GT_t,
                ...
                'BG_path': to_video_name + '{:04}.jpg'.format(BG_index)}
