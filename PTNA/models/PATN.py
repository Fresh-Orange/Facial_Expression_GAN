...
class TransferModel(BaseModel):
    ...
    def backward_G(self):
        ...
        if self.opt.with_D_PP:
            pred_fake_PP = self.netD_PP(torch.cat((self.fake_p2[:,0:3,:,:], self.input_P1[:,0:3,:,:]), 1))
    ...
    def backward_D_PP(self):
        real_PP = torch.cat((self.input_P2[:,0:3,:,:], self.input_P1[:,0:3,:,:]), 1)
        # fake_PP = self.fake_PP_pool.query(torch.cat((self.fake_p2, self.input_P1), 1))
        fake_PP = self.fake_PP_pool.query( torch.cat((self.fake_p2[:,0:3,:,:], self.input_P1[:,0:3,:,:]), 1).data )
