...
from losses.Psnr_l1Loss import Psnr_l1Loss
...
class BlendModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            elif opt.L1_type == 'psnr_l1':
                self.criterionL1 = Psnr_l1Loss(opt.lambda_A, opt.lambda_B, opt.perceptual_layers,
                                                          self.gpu_ids)
            else:
    ...
    def forward(self):
        ...
        G_input = torch.cat((self.input_P, self.input_BG, self.input_PB), 1)
        self.fake = self.netG(G_input)


    def test(self):
        self.input_P = Variable(self.input_P)
        self.input_PB = Variable(self.input_PB)
        self.input_BG = Variable(self.input_BG)
    ...
    def backward_G(self):
        ...
        self.loss_G_D1 = -torch.mean(pred_fake_D1)

        #pred_fake_D2 = self.netD2(self.fake)
        #self.loss_G_D2 = -torch.mean(pred_fake_D2)

        pred_fake_PB = self.netD_PB(torch.cat((self.input_PB, self.fake), 1))
        self.loss_G_D_PB = -torch.mean(pred_fake_PB)
        ...
        if self.opt.L1_type == 'psnr_l1':
            losses_GP = self.criterionL1(seg_P, input_seg_P_t)
            self.loss_GP = losses_GP[0] * 1.5
            # print(losses[1])
            self.loss_GP_originL1 = losses_GP[1].item() * 1.5  # .data[0]
            # print(losses[2])
            self.loss_GP_psnr = losses_GP[2].item() * 1.5  # .data[0]

            losses_GBG = self.criterionL1(seg_BG, input_seg_BG_t)
            self.loss_GBG = losses_GBG[0]
            # print(losses[1])
            self.loss_GBG_originL1 = losses_GBG[1].item()  # .data[0]
            # print(losses[2])
            self.loss_GBG_psnr = losses_GBG[2].item()  # .data[0]
        else:
        ...
        loss = self.loss_GP + self.loss_GBG + self.loss_G_D1 * self.opt.lambda_GAN + self.loss_G_D_PB * self.opt.lambda_GAN
    ...
    def backward_D_basic(self, netD, real, fake):
        # Real
        pred_real = netD(real)
        loss_D_real = torch.mean(F.relu(1. - pred_real))
        # Fake
        pred_fake = netD(fake.detach())
        loss_D_fake = torch.mean(F.relu(1. + pred_fake))
    ...
    def get_current_errors(self):
        ...
        if self.opt.L1_type == 'psnr_l1':
            ret_errors['G_P_l1'] = self.loss_GP_originL1
            ret_errors['G_BG_l1'] = self.loss_GBG_originL1
            ret_errors['G_P_psnr'] = self.loss_GP_psnr
            ret_errors['G_BG_psnr'] = self.loss_GBG_psnr
        else:
            ret_errors['G_P'] = self.loss_GP
            ret_errors['G_BG'] = self.loss_GBG
