...
class PATNModel(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect', n_downsampling=2):
        ...
        if padding_type == 'zero':
            model_stream1_down = [
                nn.Conv2d(self.input_nc_s1, ngf, kernel_size=7, padding=3,
                          bias=use_bias),
                norm_layer(ngf),
                nn.ReLU(True)]

            model_stream2_down = [
                nn.Conv2d(self.input_nc_s2, ngf, kernel_size=7, padding=3,
                          bias=use_bias),
                norm_layer(ngf),
                nn.ReLU(True)]
        else:
        ...
        for i in range(n_downsampling):
            ...
            model_stream1_up += [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                            ...
                            nn.ReLU(True)]
        if padding_type == 'zero':
            model_stream1_up += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=3)]
        else:
