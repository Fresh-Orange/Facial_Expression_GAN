...
class Psnr_l1_plus_perceptualLoss(nn.Module):
    def __init__(self, lambda_L1, lambda_perceptual, lambda_psnr, perceptual_layers, gpu_ids, percep_is_l1):
        super(Psnr_l1_plus_perceptualLoss, self).__init__()
    ...
    def forward(self, inputs, targets):
        ...
        if inputs.shape[1] == 4:
            fake_p2_norm = (inputs[:,0:3,:,:] + 1) / 2  # [-1, 1] => [0, 1]
            fake_p2_norm = (fake_p2_norm - mean) / std

            input_p2_norm = (targets[:,0:3,:,:] + 1) / 2  # [-1, 1] => [0, 1]
        ...
        loss = loss_l1 + loss_perceptual + loss_psnr

        return loss, loss_l1, loss_perceptual, loss_psnr
