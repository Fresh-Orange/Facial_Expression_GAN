...
class FaceDataset(data.Dataset):
    def __init__(self, face_dir, keypoints_dir, transform, mode, config):
        ...
        self.rotate_transform.append(T.Resize((256, 256)))
    ...
    def __getitem__(self, index):
        ...
        light = (random.random()/2 + 0.75)
        color_faceimage = TF.adjust_brightness(faceimage, light)
        contrast = (random.random() / 2 + 0.75)
        color_faceimage = TF.adjust_contrast(color_faceimage, contrast)
        saturation = (random.random() / 2 + 0.75)
        color_faceimage = TF.adjust_saturation(color_faceimage, saturation)

        return self.transform(color_faceimage), self.transform(faceimage)
