...
def knn_fusion(bg, body, body_alpha, bg_bbox, face, face_bbox):
    ...
    body = cv2.copyMakeBorder(body, 0, 0, bg_bw//4, bg_bw//4,
                              borderType=cv2.BORDER_REPLICATE)  # 身体向下填充 pad only bottom
    body = cv2.copyMakeBorder(body, 1000, 0, 1000-(bg_bw//4), 1000-(bg_bw//4), value=0, borderType=cv2.BORDER_CONSTANT)  # 身体向下填充 pad only bottom

    body_alpha = cv2.copyMakeBorder(body_alpha, 0, 1000, 0, 0,
                              borderType=cv2.BORDER_REPLICATE)  # 身体向下填充 pad only bottom
    body_alpha = cv2.copyMakeBorder(body_alpha, 0, 0, bg_bw // 4, bg_bw // 4,
                              borderType=cv2.BORDER_REPLICATE)  # 身体向下填充 pad only bottom
    body_alpha = cv2.copyMakeBorder(body_alpha, 1000, 0, 1000 - (bg_bw // 4), 1000 - (bg_bw // 4), value=0,
