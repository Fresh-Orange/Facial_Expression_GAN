...
import torch
from model.model import LandMarksDetect, RealFakeDiscriminator, IdDiscriminator
from model.model import AAExpressionGenerater as ExpressionGenerater
...
def main(config):
    ...
    loader = data_loader.get_loader("/media/data2/laixc/AI_DATA/expression_transfer/face12/crop_face",
                                         "/media/data2/laixc/AI_DATA/expression_transfer/face12/points_face", config, image_size=(160,160))
    ...
    #####  训练face2keypoint   ####
    points_G_optimizer = torch.optim.Adam(points_G.parameters(), lr=0.00001, betas=(0.5, 0.9))
    G_optimizer = torch.optim.Adam(G.parameters(), lr=0.00001, betas=(0.5, 0.9))
    D_optimizer = torch.optim.Adam(D.parameters(), lr=0.00001, betas=(0.5, 0.9))
    idD_optimizer = torch.optim.Adam(id_D.parameters(), lr=0.00001, betas=(0.5, 0.9))
    G.to(device)
    id_D.to(device)
    D.to(device)
    points_G.to(device)
    ...
    for i in range(start_iters, 150000):
        ...
        if (i + 1) % n_critic == 0:
            ...
            g_loss = lambda_keypoint * g_keypoints_loss + lambda_fake*g_fake_loss \
                      + lambda_id * g_id_loss + lambda_l1 * l1_loss
