...
import torch
from model.model import RealFakeDiscriminator, ExpressionGenerater, IdDiscriminator, KeypointDiscriminator
...
def main(config):
    ...
    for i in range(start_iters, 150000):
        ...
        faces_fake = G(faces, target_points)
        kp_fake_loss = torch.mean(kp_D(faces_fake, target_points))  # small for fake

        # Compute loss for gradient penalty.
        alpha = torch.rand(target_faces.size(0), 1, 1, 1).to(device)
        x_hat = (alpha * target_faces.data + (1 - alpha) * faces_fake.data).requires_grad_(True)
        out_src = kp_D(x_hat, target_points)
        ...
        if (i + 1) % n_critic == 0:
            ...
            faces_fake = G(faces, target_points)
            g_keypoints_loss = - torch.mean(kp_D(faces_fake, target_points))
