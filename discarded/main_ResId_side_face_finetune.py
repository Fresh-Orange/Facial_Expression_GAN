import os
import argparse
from data_loder import data_loader_sideface as data_loader
from data_loder import data_loader_augment
from torch.backends import cudnn
import torch
from model.model import LandMarksDetect, SNResRealFakeDiscriminator, FastTrackExpressionGenerater, FeatureExtractNet, SNResIdDiscriminator
...
def main(config):
    ...
    finetune_loader = data_loader_augment.get_loader("/media/data2/laixc/AI_DATA/expression_transfer/face12/crop_face",
                           "/media/data2/laixc/AI_DATA/expression_transfer/face12/points_face", config)

    loader, first_loader = data_loader.get_loader("/media/data2/laixc/AI_DATA/expression_transfer/face12/crop_sideface",
                                         "/media/data2/laixc/AI_DATA/expression_transfer/face12/points_sideface", config)
    ...
    for i in range(start_iters, 150000):
        ...
        # ID Dis
        id_real_loss = torch.mean(softplus(-id_D(first_faces, target_faces)))  # big for real
        faces_fake = G(first_faces, target_points)
        id_fake_loss = torch.mean(softplus(id_D(first_faces, faces_fake)))  # small for fake
        ...
        detecter_loss_clear = torch.mean(torch.abs(points_detect - target_points))
        ...
        if (i + 1) % n_critic == 0:
            ...
            if (i + 1) % 24 == 0 or i % 20 == 0:
                with torch.no_grad():
                    ...
                    sample_path_face = os.path.join(sample_dir, '{}-image-targetface.jpg'.format(i + 1))
                    save_image(denorm(target_face.data.cpu()), sample_path_face)
