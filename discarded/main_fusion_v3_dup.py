...
def main(config):
    ...
    loader = data_loader_fusion_v3.get_loader("/media/data2/laixc/AI_DATA/expression_transfer/face12/fusion_face", config)
    G = FusionGenerater()
    ...
    for i in range(start_iters, 100000):
        ...
        batch_size = fullfaces.size(0)
        origin_fulls = None
        for start_i in range(batch_size):
            if start_i == 0:
                origin_fulls = fullfaces[start_i]
                origin_fulls = origin_fulls.unsqueeze(0)
            # origin_knockouts = knockoutfaces[start_i]
            target_crops = cropfaces[start_i+1]
            target_crops = target_crops.unsqueeze(0)
            target_masks = maskfaces[start_i+1]
            target_masks = target_masks.unsqueeze(0)
            target_fulls = fullfaces[start_i+1]
            target_fulls = target_fulls.unsqueeze(0)

            target_crops = target_crops.to(device)
            target_masks = target_masks.to(device)
            target_fulls = target_fulls.to(device)
            origin_fulls = origin_fulls.to(device)
            #origin_knockouts = origin_knockouts.to(device)

            print("shape ", target_crops.shape)
            ...
            # Real fake Dis
            real_loss = - torch.mean(D(origin_fulls))  # big for real
            faces_fake = G(origin_fulls, target_masks, target_crops)
            fake_loss = torch.mean(D(faces_fake))  # small for fake

            # Compute loss for gradient penalty.
            # alpha = torch.rand(origin_fulls.size(0), 1, 1, 1).to(device)
            # x_hat = (alpha * origin_fulls.data + (1 - alpha) * faces_fake.data).requires_grad_(True)
            # out_src = D(x_hat)
            ...
            print("iter {} - d_real_loss {:.2}, d_fake_loss {:.2}, d_loss_gp {:.2}"
                      " g_fake_loss {:.2}, L1_loss {:.2}".format(i, real_loss.item(), fake_loss.item(),
                                                                 lambda_gp * fake_loss.item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_length', type=int, default=2)
