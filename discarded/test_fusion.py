...
import torch
from model.model import ExpressionGenerater, FusionGenerater
import sys
...
def generate_video(config, first_frm_file, json_file):
    ...
    for idx in range(len(anno)-1):
        ...
        cv2.imwrite("test_result/maskface_{}.jpg".format(idx), cv2.cvtColor(np.asarray(first_frm), cv2.COLOR_RGB2BGR))

        mask_image, Knockout_image, onlyface, img, mask_points = extract_image(first_frm, bbox, keypoint)
