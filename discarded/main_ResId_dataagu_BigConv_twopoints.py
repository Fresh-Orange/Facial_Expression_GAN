import os
import argparse
from data_loder import data_loader_augment_twopoints as data_loader
from torch.backends import cudnn
import torch
from model.model import LandMarksDetect, SNResRealFakeDiscriminator, TwoPointBigConvExpressionGenerater, FeatureExtractNet, SNResIdDiscriminator
        ...
        if (i + 1) % n_critic == 0:
            ...
            if (i + 1) % 24 == 0:
                with torch.no_grad():
                    ...
                    sample_path_target = os.path.join(sample_dir, '{}-image-rotate_point.jpg'.format(i + 1))
                    save_image(denorm(rotate_point.data.cpu()), sample_path_target)
