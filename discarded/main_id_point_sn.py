...
import torch
from model.model import LandMarksDetect, SNRealFakeDiscriminator, ExpressionGenerater, SNIdDiscriminator, SNKeypointDiscriminator
...
def main(config):
    ...
    D = SNRealFakeDiscriminator()
    #FEN = FeatureExtractNet()
    id_D = SNIdDiscriminator()
    kp_D = SNKeypointDiscriminator()
    ...
    for i in range(start_iters, 150000):
        ...
        kp_fake_loss = torch.mean(softplus(kp_D(target_faces, points_fake)))  # small for fake
        ...
        if (i + 1) % n_critic == 0:
            ...
            g_keypoints_loss = torch.mean(softplus(-kp_D(target_faces, predict_points)))
