...
import torch
from model.model import RealFakeDiscriminator, FeatureExtractNet, FusionGenerater
...
def main(config):
    ...
    for i in range(start_iters, 100000):
        ...
        target_crops = cropfaces[1:batch_size]
        target_masks = maskfaces[1:batch_size]
        target_fulls = fullfaces[1:batch_size]


        stackfullfaces = torch.stack((fullfaces[0], fullfaces[0], fullfaces[0]))
        knockoutfaces = torch.stack((knockoutfaces[0], knockoutfaces[0], knockoutfaces[0]))
        ...
        if (i + 1) % n_critic == 0:
            ...
            print("iter {} - d_real_loss {:.2}, d_fake_loss {:.2}, d_loss_gp {:.2}"
                      " g_fake_loss {:.2}, L1_loss {:.2}, feature_loss {:.2}".format(i, real_loss.item(), fake_loss.item(), lambda_gp * d_loss_gp
                                                                                                     , lambda_fake*g_fake_loss.item(),
                                                                lambda_l1 * l1_loss.item(), lambda_feature*feature_loss))
            ...
            if (i + 1) % 24 == 0:
                with torch.no_grad():
                    ...
                    sample_path_predict_full = os.path.join(sample_dir, '{}-image-origin_knockout.jpg'.format(i + 1))
